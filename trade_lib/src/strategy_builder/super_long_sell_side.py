...
def build_super_trend_results(df_bk_converted_data, period, multiplier, freq):
    ...
    default_time_frame = update_super_long_sell_side_direction(default_time_frame)
    ...
    re_name_columns = {'update_direction': 'super_long_sell_side'}
...
def update_super_long_sell_side_direction(time_frame_data):
    ...
    for index, row in time_frame_data.iloc[1:].iterrows():
        ...
        if current_dir.super_trend_direction_7_3 != 'nan' and previous_dir.super_trend_direction_7_3 != 'nan':
            if current_dir.super_trend_direction_7_3 != previous_dir.super_trend_direction_7_3:
                if current_dir.super_trend_direction_7_3 == 'down' and (compare_row is None):
                    compare_row = row
                elif current_dir.super_trend_direction_7_3 == 'down' and (compare_row is not None) and (
                        compare_row.update_price is None):
                    compare_row = row

            if (compare_row is not None) and (row.open < compare_row.close):
                time_frame_data.at[index, 'update_direction'] = 'down_entry'
            ...
            if (compare_row is not None) and (compare_row.update_price is not None) and (
                ...
                time_frame_data.at[index, 'update_direction'] = 'down_exit'
                compare_row = None

            if (compare_row is not None) and (compare_row.update_price is not None) and (
                    row.close > compare_row.high + 350):
                time_frame_data.at[index, 'update_direction'] = 'down_exit'
                compare_row = None
...
def super_long_sell_side_results(instrument_history_data, auto_inputs, instrument_name):
    default_time_frame = build_super_trend_results(instrument_history_data, period=7, multiplier=0.8, freq='5min')
    default_time_frame_col = ['date', 'super_long_sell_side']
