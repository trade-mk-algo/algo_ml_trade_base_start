...
def read_instrument_tokens(instrument, strike_price, signal, exchange, expiry_day):
    ...
    try:
        ...
        if (signal == 'BUY') or ('up' in signal):
            ...
            strike_price_list = inst_option_type_filter['Strike price']
...
def filtered_instruments(inst_split_columns, instrument):
    ...
    instruments = instruments[instruments['Scrip code'] == instrument]
    ...
    expiry_day = (instrument_days[instrument_days > date.today() + timedelta(days=5)]).head(1)
...
def download_write_instrument_tokens():
    ...
    inst_urls = pd.read_csv('resources/account/inst_links.csv')
