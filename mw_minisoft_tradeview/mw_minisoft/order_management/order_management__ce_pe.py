...
def place_instrument_orders(inst_record, tradeview_data_json):
    """
    ...
    inst_token = inst_record.instrument_name.replace(':', '_')
    inst_order_file_name = create_indicator_order_file(inst_token)
    if path.exists(inst_order_file_name):
        ...
        for user_record_position, user_record in user_records.iterrows():
            try:
                place_instrument_user_orders(inst_order_record, inst_record, user_record, tradeview_data_json)
...
def place_instrument_user_orders(inst_order_record, inst_record, user_record, tradeview_data_json):
    """
    ...
    inst_user_order_file_name = create_indicator_user_order_file(user_id, inst_token)
    if path.exists(inst_user_order_file_name):
        ...
        if inst_user_orders_last_record.shape[0] > 0:
            ...
            if diff != 0:
                ...
                place_instrument_user_orders_based_position(inst_order_record, inst_record, user_record, tradeview_data_json)
        else:
...
def place_instrument_user_orders_based_position(inst_order_last_record, inst_record, user_record, tradeview_data_json):
    """
    ...
    if user_net_positions.shape[0] > 0 and user_net_positions[user_net_positions['qty'] != 0].shape[0] > 0:
        ...
        if (user_net_positions.shape[0] > 0) and ('exit' in str(inst_order_last_record.inst_direction)):
            ...
            user_position_exit(inst_order_last_record, inst_record, user_record, user_session, tradeview_data_json)
        elif (user_net_positions.shape[0] == 0) and ('entry' in str(inst_order_last_record.inst_option_name)):
            cus_logger.info('Entering into instrument position %s As quantity zero', inst_order_last_record.inst_option_name)
            user_position_enter(inst_order_last_record, inst_record, user_record, user_session, tradeview_data_json)
    else:
